...
Character CreateCharacter(int index, int blockPosition)
{
  *double characterArray = create_character(index);
  ...
  newCharacter.name = characterArray[0];
  newCharacter.price = characterArray[1];
  newCharacter.HP = characterArray[2];
  newCharacter.isAlive = characterArray[3];
  newCharacter.attackDamage = characterArray[4];
  newCharacter.attackPerSecond = characterArray[5];
  newCharacter.rightBoundery = characterArray[6];
  newCharacter.attackTimer = characterArray[7]; // Initialize the attack timer
...
void UpdateMonsters(MonsterCharacter *monster, float deltaTime, int speed)
{
  for (int i = 0; i < charactersCount; i++)
  {
    ...
    if (monster->position.x <= characterRightBoundary && monster->row == charactersRow[i] && monster->isAlive && charactersOnField[i].isAlive)
    {
      ...
      if(charactersOnField[i].name == 4) // Bomb do AOE damage
      {
